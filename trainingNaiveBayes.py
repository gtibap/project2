...
outFilename  = 'data/naiveBayesParameters.mat'
...
varSlo = np.diag(covSlo)
varFre = np.diag(covFre)
varSpa = np.diag(covSpa)
varGer = np.diag(covGer)
varPol = np.diag(covPol)
...
sio.savemat(outFilename, {'meanSlo':meanSlo, 'meanFre':meanFre, 'meanSpa':meanSpa, 'meanGer':meanGer, 'meanPol':meanPol, 'varSlo':varSlo, 'varFre':varFre, 'varSpa':varSpa, 'varGer':varGer, 'varPol':varPol, "pslo":pslo, 'pfre':pfre, 'pspa':pspa, 'pger':pger, 'ppol':ppol})
